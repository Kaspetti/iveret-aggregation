...
def pw_fitting(
	...
	segment_boundaries = np.linspace(min_t, max_t, segments + 1)
	segments_range = np.zeros((segments, 2))

	for i in range(segments):
		segments_range[i][0] = segment_boundaries[i]
		segments_range[i][1] = segment_boundaries[i + 1]

	all_indices = np.arange(n_ts)
	split_indices = []

	for i in range(segments):
		mask = (ts >= segments_range[i][0]) & (ts <= segments_range[i][1])
		segment_indices = all_indices[mask]
		split_indices.append(segment_indices)
	...
	for i in range(segments - 1):
		mid_ts = np.repeat(segments_range[i][1], n_cs)  # Continuity at segment boundary
...
def pw_evaluate(cx: NDArray[np.floating], cy: NDArray[np.floating], cz: NDArray[np.floating], t_ranges: NDArray[np.floating], segment_detail: int = 5) -> NDArray[np.floating]:
    ...
    viz_boundaries = np.linspace(min_t, max_t, segments + 1)
    ...
    for i in range(segments):
        t_start = viz_boundaries[i]
        t_end = viz_boundaries[i + 1]
        
        ts = np.linspace(t_start, t_end, segment_detail)
...
def pw_evaluate_at_t(cx: NDArray[np.floating], cy: NDArray[np.floating], cz: NDArray[np.floating], 
					 t_ranges: NDArray[np.floating], t: float) -> NDArray[np.floating]:
	if not (cx.shape[1] == cy.shape[1] == cz.shape[1]):
		raise ValueError("Each dimension (x,y,z) must have the same number of coefficients.")
	if not (cx.shape[0] == cy.shape[0] == cz.shape[0]):
		raise ValueError("Each dimension (x,y,z) must have the same number of segments.")
	...
	if t < min_t or t > max_t:
		return np.zeros(0)
	# raise ValueError(f"t={t} is outside the valid parameter range [{min_t}, {max_t}]")

	segment_idx = None
	for i in range(segments):
		if t_ranges[i][0] <= t <= t_ranges[i][1]:
			segment_idx = i
			break

	if segment_idx is None:
		raise ValueError(f"Could not find segment containing t={t}")

	powers = np.arange(0, cx.shape[1])
	t_pow = t**powers

	pt = np.array([
		np.sum(cx[segment_idx] * t_pow),
		np.sum(cy[segment_idx] * t_pow),
		np.sum(cz[segment_idx] * t_pow)
		])
